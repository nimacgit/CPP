...
Bignumber fk, fk1(1), fk2(1);


bool isok(Bignumber & num)
{
	int s[10],e[10];
	for(int i = 0; i < 10; i++)
		s[i] = e[i] = 0;
	for(int i = 0; i < 9; i++)
	{
		if(num.digits[i] == 0 || s[num.digits[i]] == 1)
			return false;
		s[num.digits[i]]++;
	}
	int temp = num.digits.size();
	for(int i = 0; i < 9; i++)
	{
		if(num.digits[temp - 1 - i] == 0 || e[num.digits[temp - 1 - i]] == 1)
			return false;
		e[num.digits[temp - 1 - i]]++;
	}
	return true;
