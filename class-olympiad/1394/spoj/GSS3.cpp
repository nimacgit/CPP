...
nima init(const int &start,const int &end, const int &s, const int &e, const int &segnum)
{
	if(s > end || e < start)
		return seg[segnum];
	if(start == end)
	...
	nima temp[2];
	int mid = (start+end)/2;
	temp[0] = init(start, mid, s, e, 2 * segnum);
	temp[1] = init(mid + 1, end, s, e, 2 * segnum + 1);
